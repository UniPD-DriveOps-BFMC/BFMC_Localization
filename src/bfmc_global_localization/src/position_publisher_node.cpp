...
public:
  ...
  : Node("position_publisher_node")
  {
    ...
    RCLCPP_INFO(get_logger(), "Input      : %s", odom_topic_.c_str());
    RCLCPP_INFO(get_logger(), "Coordinate : %s", coordinate_topic_.c_str());
    RCLCPP_INFO(get_logger(), "Node       : %s", node_topic_.c_str());
  }

private:
  // Simple line-by-line graphml parser — handles the BFMC competition graph format.
  ...
  void odomCallback(const nav_msgs::msg::Odometry::SharedPtr msg)
  {
    ...
    // Publish nearest graph node.
    // For nearest-node lookup we use map-frame coordinates since the graph
    // is defined in the map frame regardless of GPS mode.
