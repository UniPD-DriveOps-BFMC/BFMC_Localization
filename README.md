...
| `/automobile/gps/tag_pose` | `bfmc_gps_position/GpsTagPose` | GPS team | fields: `x`, `y`, `z` (metres), `quality` (0–100) |
| `/oak/left/image_mono` + `/oak/right/image_mono` | `sensor_msgs/Image` | Camera driver | stereo mode |
| `/oak/rgb/image_raw` + `/oak/rgb/camera_info` | `sensor_msgs/Image` | Camera driver | RGB + intrinsics |
| `/traffic/detection` | `std_msgs/String` | Detection team | label of detected sign (e.g. `"stop"`, `"crosswalk"`) |
| `/traffic/distance` | `std_msgs/Float32` | Detection team | distance in metres to the detected sign |
...
| Topic | Type | Meaning |
|-------|------|---------|
| `/odometry/local` | `nav_msgs/Odometry` | Position + velocity relative to start position |
| `/odometry/global` | `nav_msgs/Odometry` | Position + velocity relative to GPS (0, 0). Falls back to `/odometry/local` if GPS is off |
| `/automobile/current_coordinate` | `geometry_msgs/PoseStamped` | X, Y in metres relative to GPS (0, 0). Falls back to start position if GPS is off |
| `/automobile/current_node` | `std_msgs/Int32` | Current node ID in the competition track GraphML map |
| `/automobile/sign/base_pose` | `geometry_msgs/PoseWithCovarianceStamped` | Detected sign pose in map frame |
...
/traffic/distance          ┘                             ──► /automobile/sign/base_pose
```
...
| SUB | `/traffic/detection` | `std_msgs/String` |
| SUB | `/traffic/distance` | `std_msgs/Float32` |
